...
Specifically, we reproduce results from a collaborative project between UCL
...
efficacy in pathologies ranging from cancer to inflammation. We demonstrate the 
use of a framework called HTBAC, designed to support
...
enables the selection of simulation parameters during runtime which can, in
principle, optimize the use of computational resources whilst producing results
within a target uncertainty.
...
system. Given the very large space of possibilities, a signficant number of
...
protocal may converge at different rates; the instance that has converged
...
calculations. HTBAC facilitates the execution of multiple simulations
\jhanote{simulations, protocols?} while supporting the adaptive execution of
algorithms. \jhanote{Isn't the following just an example of adaptivity? If so,
integrate into previous sentence} HTBAC enables the selection of experimental
...
% usable software systems, (ii) scale and interoperability, and (iii) adaptivity
% challenges. }\mtnote{Better?}

% As the nature of scientific inquiry and the applications to support that
% inquiry evolve, there is a need to support the execution of
...
\jhanote{(i) eliminate this as a stand alone section; (ii) merge with the
opening of this section, as there is overlap \& also repetition.}
...
Titan for different protocols. \jhanote{this point has been repeated 3 or 4 time, and reference made to figure $>$ once. Please fix.}
...
Our ensemble \jhanote{not sure what our ensemble means} has already provided
...
uncertainties much larger campaigns are needed.

\jhanote{please integrate the above and the following paragraphs}
...
size of target system and number of ensemble simulations. 

\jhanote{should this be another paragraph / start?} In addition, HTBAC enabled
the adaptive execution of the TIES protocol providing greater convergence
(i.e., lower errors) for a given amount of computational resources.
...
HTBAC, each system was run by hand. 

\jhanote{Now shoulnd't we discuss time take for adaptive approach??}
...
... HTBAC satisfies this need by facilitating the routine of running studies 
requiring sustained usage of millions of core hours per day. \jhanote{discuss}
