...
scales almost perfectly to hundreds of concurrent multi-stage pipelines in \ref{fig:weak_scaling} where we show the weak scaling properties of TIES. 
...
\begin{figure}
  ...
  weak scaling of TIES protocols. We ran two trials for each protocol instance 
...
\jhanote{Adaptivity has to be motivated better. Currently it is almost a
posteriori..}
...
\jhanote{Would the last paragraph be placed in Section V or VI?}
...
\jhanote{Dave/Kristof: We are missing a "comparison to related approaches"}
...
As previously described this type of study does not meet the size necessary to really impact industrial processes which focus from libraries of potentially millions of compounds doen to hundreds of hits which need to be optimized.
