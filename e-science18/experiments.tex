...
\begin{itemize}
    ...
    run on the CI. 
    \item \textbf{HTBAC Overhead}: Time taken to process the application, instantiate the 
    components and subcomponents, translate protocols into pipelines for EnTK, 
    and validate resource descriptions. 
    \item \textbf{EnTK and RP Overhead}: Time taken by ensemble management system and the 
    runtime system to submit and manage the execution of tasks. 
    \item \textbf{\texttt{aprun} Overhead}: Time taken by \texttt{aprun} to schedule
...
instances as shown in Figure~\ref{fig:ws} (a), we account for
additional time that is factored into Task Execution Time. The increase in 
\texttt{aprun} overhead occurs when tasks show failure which introduces 
additional communication between the nodes that run the job (i.e. MOM Nodes) and 
an attempt to relaunch failed tasks.  
...
impact on TTX/TTC/Resource utilization.}\jdnote{added a technical detail}
...
the number of tasks as observed in Figure~\ref{fig:ws} (a)--(c), bars in 
blue color.
...
analysis?}\jdnote{It can be left out, I was just trying to explain the barriers
in EnTK between stages that may incur on the overheads, given that all 
simulations need to finish before moving on to next stage. With protocols that
have more tasks/stage, I thought this should be mentioned as a contribution.}
% ------------------------- COMMENTED BY MT -------------------------
...
In comparison to Task Execution Time, the EnTK and RP overheads are 
an order of magnitude shorter, yet contribute directly to the total duration of 
the application. Ultimately, in a computational drug campaign, the use of 
runtime systems results in additional allocation of resources. In situations where
campaigns are limited by a computational budget, we approximate those overheads 
would contribute to $\approx15\%$, based on the data in Figure~\ref{fig:ws}.
For such campaigns, efficient utilization of resources is most crucial 
consideration. In the scope of this work, we leverage the efficiency of 
resources as provided by the runtime system, in order to focus on scalable 
execution of adaptive methods that can improve accuracy and reduce time to 
solution. \mtnote{I am afraid this whole paragraph needed to be
