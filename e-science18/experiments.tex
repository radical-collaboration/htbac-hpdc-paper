...
\begin{itemize}
    \item \textbf{Task Execution Time}: Time taken by the task executables to
    run on the computing infrastructure (CI).
    ...
    schedule tasks on a set of compute nodes.\mtnote{Are we sure aprun
    `schedules' tasks? Maybe launch as we call `aprun' a launch method?}
\end{itemize}
...
supported by \texttt{aprun} (\mtnote{reference the subsection of the paper
where this limit is described}). Nevertheless, we include results at this
...
duration.} Task Execution Time to be fixed as the number of protocol
...
overhead is negligible when compared to \mtnote{total?} Task Execution Time.
...
application \mtnote{protocol?}. Among these components, the task scheduler of
...
depends on the total number of tasks that need to be scheduled.\mtnote{Are we
