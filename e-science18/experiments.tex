...
iteration count? Needs complete rewrite.}
Fig.~\ref{fig:adapconv} shows progression of the algorithm, starting from 3
initial lambda windows, and at each iteration bisecting the interval where
the error is above the set threshold. For system \jhanote{X -- fix!!} we can see as how the
...
"protocol" } protocol implemented in HTBAC, offers dynamic termination for any
...
\begin{figure}
  ...
  simulation. \jhanote{Reviwer will want to know what happens with remaining two candidates?}}
