...
strong scaling experiment (4--5), we change the ration between resources
...
overheads of HTBAC, EnTK and RP. This offers insight about the impact of
HTBAC and its middleware on the time to completion of each workload.
...
simulation stage $S4$ executes each simulation for 4ns, while stage $S5$
consists of 5 analysis tasks which aggregate the simulation results of $S4$.
The global analysis stage $S6$ contains a single task that aggregates the
results from $S5$.
...
divided into 4 sub-stages of 500000 time-steps each. \jdnote{should we
...
versions of the NAMD-MPI MD engine, compiled according to the capabilities of
each environment and of NAMD itself, and launched via the \texttt{aprun}
command. For the analysis stages in the TIES protocol we used AmberTools.
...
compute node) even if our performance measurement of NAMD on Blue Waters
indicated that 16 cores would offer the best trade-off between computing time
and communication overhead. \jdnote{Should we include the scalability plot of
...
properties. The durations we measure are defined as:
...
\begin{itemize}
    ...
    \item EnTK and RP Overhead: Time taken by EnTK and the runtime system to 
    submit and manage the execution of tasks. 
    \item \texttt{aprun} Overhead: Time taken by \texttt{aprun} to schedule
    tasks on a set of compute nodes. 
\end{itemize} 

% \jdnote{So the TTX here is measured with events measuring the duration 
% between cu exec start and cu exec stop, since we use APRUN we cannot look at 
% events like app_start until app_stop, we use the term TTX internally, but maybe 
% for this paper we should call it task execution duration}
...
Once the runtime system relinquishes control to \texttt{aprun}, the precise 
time duration of when \texttt{aprun} schedules the task on the compute
nodes and MD kernel beginning execution is unknown. Therefore, we make an 
approximation. We obtain the \texttt{aprun} overhead by measuring the 
difference between TTX and the time duration produced by the \texttt{NAMD} logs. 
...
execute beyond the average limit of 450 concurrent tasks. Nevertheless, we 
include results at this scale to show HTBAC's performance capabilities in 
supporting higher orders of concurrent simulations. 

Experiment 2 in Figure~\ref{fig:ws} (b) measures weak scalability with the ESMACS
...
observe TEX showing ideal weak scaling behavior 
