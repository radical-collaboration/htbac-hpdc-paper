...
$\lambda$ values. We evaluate the total number of $\lambda$ windows as
...
The lack of adaptivity in free energy calculation workflows usually comes down
to the way simulation engines (like NAMD, Gromacs, Amber, etc.) work. The user
...
implement at scale. That is why we have fewer adaptive algorithms than one
might suspect. The opening line says the cause of the lack of adaptivity is 
...
schemes. Utilizing adaptivity results in faster convergence rate and more
accurate results.

\jhanote{Once again, we need to be careful: there is conflation between cause and effect. Adatpvitiy is not a principle in of itself so has not intrinsic value standalone. what is important
is faster convergence and more accurate results. adaptivity is one route to
obtain them. The way the previous sentence is written conflates effect with
driver.}


The aim of introducing adaptive quadrature \jhanote{adaptive quadrature is
raised several times it isn't describe anywhire}  to alchemical free energy
...
\SI{77}{\percent} (see figure~\ref{fig:savings}). More importantly, the error on all of the systems are reduced
to below \SI{0.2}{\kilo\calorie\per\mole}, which has recently been shown to be
the upper bound of reproducibility across different simulation
