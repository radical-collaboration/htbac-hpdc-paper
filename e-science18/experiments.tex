...
In all weak scaling experiments (Figure~\ref{fig:ws}) we observe Total Task
Execution Time to be fixed as the number of protocol increases, thereby
showing ideal weak scaling behavior.
...
simulation engines (like NAMD, Gromacs, Amber and others) work. The user
hand-writes a configuration file and, together with the system description
(topology, PDB), passes it to a binary executable, waiting for the simulation
to end before analyzing the results. There is a disconnect between the
simulation run and the analysis, i.e. the intermediate results can have no
influence over the simulation \emph{after} it has been started. Therefore,
certain decisions, like number of time steps to run for have to be made
\emph{a priori}, and the continuous accumulation of results can, by
definition, have no effect of these parameters.

Having the mobility and dynamism that HTBAC offers, we are able to enhance
our protocols, using adaptive schemes. Utilizing adaptivity results in faster
convergence rate and more accurate results.
...
engines~\cite{}.\mtnote{missing reference}
...
\begin{figure}
  ...
  to fulfill.\mtnote{This figure in unreferenced in the text.}}
  \label{fig:savings}
\end{figure}
...
\begin{figure}
  ...
  simulation.\mtnote{This figure in unreferenced in the text.}}
  \label{fig:termination}
\end{figure}

\begin{table}
  \caption{Simulation time of non-adaptive and adaptively terminated runs for
  a given convergence criterion.\mtnote{This table in unreferenced in the
  text.}}
  \label{tab:adapterm}
  \centering
