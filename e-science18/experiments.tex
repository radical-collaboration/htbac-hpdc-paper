...
pre-assigned $\lambda$ windows. As described in the TIES workflow diagram 
X, this produces a total of 65 concurrent simulations for stages $S1$ - $S4$. 
The production simulation stage $S4$ executes each simulation for 4 nanoseconds. 
The analysis stage, $S5$, assigns five analysis tasks which aggregate all 
simulation results of $S4$ with respect to each replica and to calculate the 
adaptive quadrature...The last stage $S6$ 
assigns 1 task to perform a global analysis aggregating the results from $S5$. 
In the adaptive quadratures, we alter the number of $\lambda$ windows. First, 
we break down $S4$ into 4 stages, $S4.1$ - $S4.4$, where each stage shorter 
production simulations of 1 nanosecond. We initially assign X $\lambda$ windows for stage 
$S4.1$...
Once each simulation is complete within a
