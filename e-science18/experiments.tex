...
\jdnote{mentioned lower bound on concurrent number of simulations}
...
previously published results cited in the literature. \mtnote{what does this 
mean? We cited the results already in the paper? These results are published?}
...
\jdnote{I meant to say we validate our free energy calculation results from this 
paper with known results that are available in the chemistry literature. Does it
make more sense now?}

Given that protocols like TIES are more computationally demanding than ESMACS
\mtnote{Grammar: more than?}\jdnote{more than ESMACS}, it is paramount to use 
...
experiments\ldots We used m protocols\ldots See Table o\ldots''. Moved a paragraph here as a starting point.}\jdnote{provided ``segway"}
...
specify what type of methods are we referring to)} 
by increasing the number
of protocol instances while keeping the required 
\mtnote{required by
what/whom? ``keeping the number of pipelines fixed at the number required by
\ldots'' or something like that?} number of pipelines. 
\jdnote{I've changed this introduction describing ws, does it make sense?}
...
and reducing \ldots''} 
\jdnote{also changed this introduction to ss.}
...
\mtnote{different from what?}\jdnote{addressed}
...
\jdnote{addressed}, 
we reduce the number of time-steps omit the analysis steps $S5$ and $S6$, as the
focus of scalability is not based on duration but rather characterization of 
performance. We assign the following time-steps: $S1$: 1000; $S2$: 5000; $S3$: 
5000; $S4$: 50000. 
...
During our scalability experiments we found two main limitations of 
\texttt{aprun}. First, the number of
...
therefore write.} \jdnote{addressed}
...
for adaptive termination.}\mtnote{I agree and both approaches would work for
me. We may want to be consistent across all experiments though: either we use
a single subsection illustrating the results of all the experiment or we
discuss the result of each experiment as we introduce it.}
...
non-adaptive in \mtnote{to? The sentence would benefit from an iteration}
...
Figure~\ref{fig:adapquad} \mtnote{missing figure? There is a table with the
label `adapquad' but not a figure.} shows progression of the algorithm,
