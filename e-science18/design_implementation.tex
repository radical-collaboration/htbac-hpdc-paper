...
management from the user. Figure~\ref{fig:components} shows the components
...
Once one or more adaptive or nonadaptive protocols are described in HTBAC
(Figure~\ref{fig:architecture}.1), the Runner component tags each protocol
instance with a unique ID and converts protocols into EnTK pipelines of
stages of tasks (Figure~\ref{fig:architecture}.2). The Descriptor 
aggregates the application description, including protocols
(Figure~\ref{fig:architecture}.3) and their simulation conditions, 
(Figure~\ref{fig:architecture}.4)

% , and information about computing
% infrastructure requirements (Figure~\ref{fig:architecture}.5).

The Descriptor passes the protocol description to the Protocol
Manager to generate the protocol with the appropriate simulation 
conditions (Figure~\ref{fig:architecture}.7). The Runner 
aggregates all protocols (Figure~\ref{fig:architecture}.8) and passes the 
protocols as well as the application's resource description that contains 
information about computing infrastructure requirements to the ensemble 
management system (Figure~\ref{fig:architecture}.9).
\mtnote{This is an example of a meaningful
numbering for the architecture diagram. Steps 3, 4, and 5 could be a single
one, depending on the granularity of the diagram.}

Pipelines are submitted to EnTK's Application Manager component which sets up
...
cores, queue, and user credentials. The Runner passes this resource
description to EnTK which uses these requirements to derive a resource
request that is then converted into a pilot description for RP. RP converts
this pilot requests into a batch script that is submitted to a suitable HPC
machine. Once the pilot becomes active, it pulls the tasks made available by
EnTK in bulk, executing them on the pilot resources~\cite{merzky2015radical}.
