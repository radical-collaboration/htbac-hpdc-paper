...
\jdnote{I created another section that comes after this section. }
...
stages of tasks (Figure~\ref{fig:architecture}.2). The Runner aggregates the
application description, including protocols
(Figure~\ref{fig:architecture}.3), their simulation conditions
(Figure~\ref{fig:architecture}.4), and information about computing
...
Manager (Figure~\ref{fig:architecture}.6) which communicates with the Step
Manager and the Ensemble-Member Manager to generate the protocol with the
appropriate simulation conditions (Figure~\ref{fig:architecture}.7). The
Application Generator aggregates all protocols
(Figure~\ref{fig:architecture}.8) and passes the protocols as well as the
application's resource configuration to the ensemble management system
(Figure~\ref{fig:architecture}.9).\mtnote{This is an example of a meaningful
...
The resource requirements as specified by the user in HTBAC which include
