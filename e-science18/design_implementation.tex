HTBAC serves as a software tool that enables the scalable and adaptive
exploration of ensemble-based free energy studies \mtnote{protocols?}.
HTBAC is based on a co-design which integrates users i.e. domain experts and
developers to elicit requirements\mtnote{I am not sure I understand what this
...
protocols,\mtnote{``from'' instead of ``,''? See also comment in the
following paragraph} execution and resource management; and, (3) provide
...
Based on the requirements of the computational drug campaign \mtnote{which
one?} which requires the ability to run multiple concurrent protocols in
...
HTBAC.\mtnote{I am not sure I understand this sentence: grammar
...
focus on WMS and RTS? Are WMS and RTS components of HTBAC?} The RTS has to
...
without supporting punctuation} protocols that differ in computational
...
HTBAC thereby requires an adaptive runtime systems\mtnote{system} that
...
We model an application of HTBAC\mtnote{what is an ``application of HTBAC''?
`In' (instead of `of') HTBAC?} using the following user-facing constructs:
...
protocol represents a physical system i.e. drug candidate \mtnote{see the
previous comment about using `i.e.'}. A protocol defines a % simulation
...
create \textbf{workflows}...\mtnote{Missing sentence?}

The \textbf{Protocol} class \mtnote{we used `construct' before, as a synonym
of abstraction} enables multiple instantiations of protocol types such as
ESMACS or TIES. Each protocol follows the same initial sequence of simulation
steps i.e. minimization, equilibration and production simulation\mtnote{see
the previous comment about using `i.e.'}. Protocols differ only by their
...
The \textbf{Simulation} class \mtnote{we used `construct' before, as a
synonym of abstraction} specifies simulation parameters that shape the
...
(proper names are not plural) without differentiating them (it?) from thoese
