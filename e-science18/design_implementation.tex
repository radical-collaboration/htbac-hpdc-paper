...
% in terms of ensemble applications:

% \begin{itemize}
%   \item \textbf{Protocol:} Abstraction of a free energy protocol.
%   \item \textbf{Simulation:} Simulation parameters for a protocol.
%   \item \textbf{Analysis:} Analysis parameters for a protocol.
%   \item \textbf{Resource:} Abstraction to describe resources requirements.
% \end{itemize}
...
% and the amount of time for which these resources should be available. 
...
% HTBAC abstracts the protocol coordination, resource management and
% execution management from the user.
...
% Using a design based on connectors allows to isolate middleware-specific
% capabilities within well-defined architectural components.

% For example, in the case of EnTK, the application is converted into and
% ensemble of pipelines, stages and tasks.
...
The Middleware Connector of the Runner gets the application description and
converts it into a middleware-specific description, i.e., into EnTK pipelines
of stages of tasks. The connector also creates a middleware-specific resource
request and submits it to the underlying execution layer
(Figure~\ref{fig:architecture}.4). Once the resources are acquired, the
middleware of the execution layer communicates the availability to the
connector (Figure~\ref{fig:architecture}.5) that initiate the execution.
...
Once the pilot becomes active, EnTK identifies those application tasks that
...
% Support for intra-protocol adaptive execution requires an adaptive
% execution strategy. In HTBAC, we leverage the adaptive capabilities
% provided by EnTK~\cite{adaptivebiomolecular} to implement such a strategy.
...
% redistributed across simulations as, by design of ensemble-based free
% energy protocols, all simulations within a protocol require the same amount
% of resources.
