...
to the logic of the adaptive algorithms. In this way, HTBAC will
...
Each protocol models a unique protein ligand physical system. Protocols follow
a sequence of simulation and analysis steps, assigning ensemble members to 
execute independent simulations or analysis. An ensemble member that executes an 
independent simulation within a simulation step is referred to as a replica. 
Each simulation is assigned a different initial velocity, which enables 
simulations to begin in different parts of the ligand's phase space.
...
Fig.~\ref{fig:architecture} shows the components and subcomponents of
HTBAC. The API enables users to codify protocol descriptions in terms of 
protocol type, simulation and analysis steps, and compute infrastructure
...
% Fig.~\ref{fig:pst} provides the visual implementation of the TIES
...
% about algorithms, none of which is specific to their implementation in
% HTBAC. I think we need a different section that discusses how HTBAC is used
% to support adaptivity in the context of TIES.}
...
%      for Adaptive Execution where I've modified the text to include a
%      snippet that captures an adaptive application.\\
%   -- Certain execution details will need be captured in the experiment
%      design for adaptivity experiments, i.e., what is the initial pilot
%      size and how we redistribute resources}
...
% subsections and, maybe, the last one about implementing adaptivity (I can
% see why to keep that here though)?}
...
% Fig.~\ref{fig:adaptive_ties} shows our implementation of TIES as a
...
% C -------------------------------------------------------------------------
\subsection{Adaptive Execution Model}\label{ssec:adaptive_execution}
...
% to generate new simulation steps based on current simulation results.
...
changing the application. If new simulations are to be generated, the Execution
Manager bypasses termination of the application, and passes the added 
application description to the connector. 
...
redistributes an even share of the total requested cores to each simulation. RP 
allows for new simulations to execute within the job's wall-time, without 
acquiring new resources. This is to avoid submitting the new application 
requirements to the batch queue.
...
% (Fig.~\ref{fig:architecture}.7). The Runner aggregates all protocols
% (Fig.~\ref{fig:architecture}.8) and passes the protocols as well as the
...
% earlier. Fig.~\ref{fig:adaptive_ties} illustrates the adaptive workflow
