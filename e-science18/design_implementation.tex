...
protocols. In addition, an abstraction is needed to encode adaptivity. 
...
\jdnote{introducing EnTK as workload 
management system requires the definition of workload}
...
Once the workflow is described in HTBAC, 
...
definition of workflows}, protocols are encoded into EnTK pipelines. 
...
We demonstrate the implementation and execution of an adaptive workflow using 
the TIES protocol. In TIES the workflow consists of an iterative pipeline in two 
stages: ensemble simulation and adaptive quadratures to determine optimal 
placement of further simulations. 
...
Each pipeline is tagged with an ID. During re-execution we reintroduce
the previous pipelines and incorporate new pipelines, in order to continue
...
of additional tasks.\mtnote{I tried to fix a bit the paragraph but it still
...
By design of free energy protocols, some protocols are amenable and produce 
higher accuracy by running simulations that are discovered during runtime. 
...
The ESMACS protocol contains 4 simulations steps: minimization, initial 
equilibration, secondary equilibration, and MD simulation.  
