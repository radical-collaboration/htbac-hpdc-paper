...
compounds in order to design new therapeutics for clinical insight. 

Recent development shows increasing interest in the use of molecular simulation to 
...
However creating simulation protocols which have well-defined uncertainty and 
produce statistically meaningful results represents a significant computational 
challenge. 

Differences amongst investigated candidates will demand different 
protocols as studies progress. For example, drug design campaigns often require 
an initial screening of candidate compounds to filter out the worst 
...
different problem spaces. For example, advanced sampling is a type of adaptive
algorithms that guide simulations toward improve sampling quality by steering 
execution towards more interesting phase spaces (cite). 
Metadynamics and expanded ensemble dynamics are another class of 
well-established adaptive algorithms where individual simulations jump between 
