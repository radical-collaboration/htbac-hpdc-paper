...
\jhanote{this idea needs better contextualization and motivation. at a minimum
references will be needed to substantiate the claim.} Specifically, ensemble-
...
\jhanote{It is unclear what "these developments" are} These developments are resulting in
...
\jhanote{uncertainty in what?} and produce statistically meaningful results.
This continues to represent a significant computational challenge. \jhanote{In
previous paragraph, it is unclear what "technology" is being referred to, and
what  "approaches" are being referred to.}
...
requirements may change. \jhanote{Is a study, a set of simulatixons? a specific
simulation? A campaign?}
...
% <<<<<<< HEAD Moreover, the "fire and forget" mentality no longer fulfills
% computational efficiency when campaigns are limited by a computational
% budget or require timely insight for clinical efficacy.
% =======
...
\jhanote{This is a definition of an adaptive workload, not an adaptive
application or adaptive algorithm. Most people who develop applications and
algorithms don't think in terms of task graphs!}

\jhanote{Why is this in the introduction and not related / background work?}
...
for a drug candidate. \jhanote{Is the purpose of the following to motivate
new adaptive sampling algorithms?}
...
well with this? the wang2015 paper only mentions FEP}.
...
completion and computational cost. \jhanote{convergence of what? i don't think
we've identified what convergence here is?}
...
\jhanote{Now focus on
HPC Challenges for Adaptivity, instead of revisiting a point that has been
made 3 times. Also, I don't know what enhancements in adaptive algorithms you
are referring to. What is the problem / limitation that we are trying to
enhance?} \sout{For molecular simulations to truly influence decision making
...
high throughput binding affinity calculations at scale. \jhanote{OK so there
is missing software. But what is the technical challenge or missing
capabiliteis that this software must provide? The reader is unable to discern
that so far. Also the challenges / missing capabilities must be clear before
going into a discussion of HTBAC, c.f.. paragraph which begins with
"In order to support.."}
...
\jhanote{Is this paper about HTBAC or about the methodological (not science) advances that come about using HTBAC?} 
