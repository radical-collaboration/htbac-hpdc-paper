...
\jhanote{I think we should discuss the design of HTBAC before discussing
implementation}
...
\jhanote{Why do we not call the second component "protocol". Workflow seems to
be both misleading and too general.} The \textbf{Workflow} component is a
container that implements the ESMACS and TIES as EnTK workflows. \jhanote{What
is an EnTK workflow, as opposed to a HTBAC workflow? c.f. first line} The
...
in order to study a range of physical systems. \jhanote{It is unclear "what"
allows for multiple protocol instances to be specified in a single run.}
\jhanote{Not sure why the physical system should be clubbed into the protocol
abstraction/component? clearly this is something that is already done in EnTK and
elsewhere, so nothing specific to the protocol itself}

The theory and benefits of running multiple simulation has been documented in
literature (cite). We think this \jhanote{What is "this"?} is a crutial part
...
runs. \jhanote{As discussed in SC17 paper before, consistency in use of
replica, ensemble, multiple simulation will reduce confusion.} With HTBAC,
...
collecting data from them, for later analysis. \jhanote{Why is Replica in bold
face? Is it an explicit object/class? Or is it a parameter/configuration?}

\jhanote{Is this specific to the TIES protocol or to the protocol class. Needs
clarification.} Furthermore, the TIES protocol enables the user to control the
...
\jhanote{Needs a much cleaner rewrite.}
