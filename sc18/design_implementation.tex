...
execution of concurrent multi-stage pipelines, flexibly partitioned across
different protocols, (2) abstract the complexity of building protocols,
execution and resource management; and, (3) provide adaptive features (i.e.,
modifying the task-graph during runtime, based on previous tasks), to enable
...
ESMACS or TIES. For TIES, each protocol typically corresponds to a single 
physical system, and multiple instances can be supported. The TIES protocol 
...
task-count adaptivity which we define as runtime changes to the number of tasks in
