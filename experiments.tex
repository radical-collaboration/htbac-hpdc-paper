...
details from design of experiments} Each of the simulation stages contains a
task for every unique $\lambda$-replica combination.

\begin{figure}
  \centering
   \includegraphics[width=\columnwidth]{figures/Adaptive_TIES.png}
  \caption{\jhanote{Please provide caption}}
...
stage, $s4$ \jhanote{not defined. where is the figure and accompanying
description?} executes a four ns simulation duration. The analysis stages of
...
In the adaptive workflow, over the course of a run \jhanote{protocol
instance?} we alter the number of $\lambda$ windows being simulated. The
...
\jhanote{consistency in the description of stages: either boldface ot tttext}
